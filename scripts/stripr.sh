   ...
   echo $file
	...
	if [ -z "$audio" ] 
			then
				 ...
				 # mv "$1" /media/Trash/;
					if [ $APP_TOKEN != "YOUR_TOKEN_HERE" ] #Don't modify
					then 
						wget https://api.pushover.net/1/messages.json --post-data="token=$APP_TOKEN&user=$USER_TOKEN=$file - Foreign movie processed!&title=RadarrM" -qO- > /dev/null 2>&1 &
					fi
			else
				...
				then
					...
					if [ $APP_TOKEN != "YOUR_TOKEN_HERE" ] #Don't modify
					then 
						wget https://api.pushover.net/1/messages.json --post-data="token=$APP_TOKEN&user=$USER_TOKEN=$file - Audio or Subtitle removed!&title=RadarrM" -qO- > /dev/null 2>&1 &
					fi
