...
audio=$(echo "$DETAILS" | jq '[.tracks[] | select (.type=="audio" and (.properties.language | test("eng|por|kor|jpn|und"))) | select (.codec | test("TruHV|AC9|DTZ"; "i") | not) | .id] | map(tostring) | join(",")' | cut -f2 -d\")
